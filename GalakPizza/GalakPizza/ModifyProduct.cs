...
namespace GalakPizza
{
    public partial class ModifyProduct : UserControl
    {
        ...
        private void chooseProduct(object sender, EventArgs e)
        {
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    ...
                    listcategory.SelectedItem = category[dr.GetInt32(1)];
                }
            }
        ...
        void modifyProduct(object sender, EventArgs e)
        {
            ...
            {
                query = $"update Dodatki set Nazwa='{name.Text}', IDKategoriiDodatku={categoryID}, Cena=convert(decimal(5,2), replace('{price.Text}',',','.')), Zdjecie='{imagePath.Text}', Dostepny={availabe.SelectedIndex} where IDDodatku={id}";
            }
            else
            {
